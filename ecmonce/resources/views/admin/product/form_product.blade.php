...
<div class="form-group{{ $errors->has('number') ? ' has-error' : '' }}">
    {{ Form::label('number', trans('product.lbl-number'), ['class' => 'col-md-4 control-label']) }}
    <div class="col-md-6">
        {{ Form::number('number', isset($product->number) ? $product->number : old('number'), ['class' => 'form-control', 'id' => 'number', 'required' => true]) }}
    </div>
price
...
<div class="form-group{{ $errors->has('made_in') ? ' has-error' : '' }}">
    ...
    <div class="col-md-6">
        {{ Form::number('made_in', isset($product->made_in) ? $product->made_in : old('made_in'), ['class' => 'form-control', 'id' => 'made_in', 'required' => true]) }}
    </div>
price
...
<div class="form-group{{ $errors->has('category') ? ' has-error' : '' }}">
    ...
    <div class="col-md-6">
        {{ Form::select('role', [config('setting.role.user') => 'User' , $categories, isset($product->category_id) ? $product->category_id : old('categor'), ['class' => 'form-control', 'id' => 'category']) }}
    </div>
</div>
